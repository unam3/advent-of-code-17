const input = `62	1649	1731	76	51	1295	349	719	52	1984	2015	2171	981	1809	181	1715
...
const taskSpreadsheet = input.split('\n')
...
const testSpreadsheet = testInput.split('\n')
    .map(
        rowString => rowString.split(' ')
            .map(string => Number(string))
    )

const getSmallestLargestDifference = (row) => {
    ...
    row.forEach(
        (atom, step) => {
            if (!smallest) {
                smallest = atom
            } else if (atom < smallest) {
                if (!largest) largest = smallest

                smallest = atom
            } else if (atom > largest || !largest) {
                largest = atom
            }

            //console.log(atom, step, 'l', largest, 's', smallest)
        }
    )

    //console.log('l', largest, 's', smallest, 'd', largest - smallest)

    return largest - smallest
}

const getChecksum = (spreadsheet) => spreadsheet.reduce(
    (checksum, row) => checksum + getSmallestLargestDifference(row),
    0
)

console.log(
    'test spreadsheet checksum is', getChecksum(testSpreadsheet), '; ',
    'task spreadsheet checksum is', getChecksum(taskSpreadsheet)
)
