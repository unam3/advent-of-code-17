...
const parseGroup = (str, level = 1) => {
    ...
    if (parsedContent) {
        const groupContent = groupsCount === 1 ?
            parsedContent[1].replace(/[^!]<.*?[^!]>/g, '{') :
            parsedContent[1]
            ...
            (chunk) => {
                ...
                if (chunk.startsWith('{')) {
                    if (garbageHasNoEnd) {
                        ...
                        parseGroup(chunk, level + 1)

                        return true
                    }
                } else {
                    garbageHasNoEnd = chunk.endsWith('>') ? false : true
                }
            }
        )
...
const doTests = () => {
    ...
    console.log(
        ...
        streamSamples.map(
            stream => {groupsCount = 0; return getTotalScore(stream).groupsCount}
...
console.log(
    JSON.stringify(
        scoreCountSamples.map(
            ([stream, referenceScore]) => {
                groupsCount = 0;
                score = 0;
                
                return {...getTotalScore(stream), referenceScore};
            }
        ),
        null,
        2
