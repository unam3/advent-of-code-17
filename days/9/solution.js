...
const parseGroup = (str) => {
    ...
    const parsedContent = /{(.*)}/.exec(str)

    if (parsedContent) {
        const groupContent = groupsCount === 1 ? parsedContent[1].replace(/[^!]<.*?[^!]>/g, '{') : parsedContent[1]

        const rawSplittedContent = groupContent.split(',')

        let garbageHasNoEnd

        const splittedGroups = rawSplittedContent.filter(
            (chunk) => {
                // in the group there is no escape (exclamation sign) allowed, so it's always ends with '}'
                if (chunk.startsWith('{')) {
                    if (garbageHasNoEnd) {
                        if (chunk.endsWith('>') && !chunk.endsWith('!>')) {
                            garbageHasNoEnd = false
                        }
                    } else {
                        parseGroup(chunk)
                        //parseGroup(chunk.replace(/[^!]<.*[^!]>/g, ''))

                        return true
                    }
                } else {
                    //if (chunk.startsWith('<')) {
                    //    // if chunk ends with > then it's the end of garbage
                    //    // if it's not - we need to track the end in the next chunks
                        if (chunk.endsWith('>')) {
                            garbageHasNoEnd = false
                        //} else {
                        //    garbageHasNoEnd = true
                        } else
                    //}
        ...
        // for debug purposes
        //return {groupsCount, splittedGroups}
...
const getTotalScore = (input) => {
    ...
    return parseGroup(removeGarbage(input))
}

//console.log(JSON.stringify(getTotalScore(
//    exampleInput
//    //puzzleInput
//)))
...
const doTests = () => {
    console.log(
        'all garbage must be ignored, so only one group must be detected:',
        getTotalScore(groupWithGarbage) === 1
    )

    console.log(
        'all stream samples was processed correctly:',
        streamSamples.map(
            stream => {groupsCount = 0; return getTotalScore(stream)}
...
//console.log(
//    JSON.stringify(
//        streamSamples.map(stream => {groupsCount = 0; return getTotalScore(stream)}), null, 2
//    )
//}
